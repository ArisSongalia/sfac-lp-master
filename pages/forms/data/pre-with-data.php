...
LEFT JOIN tbl_schoolyears ON tbl_schoolyears.stud_id = tbl_students.stud_id
    ...
    where tbl_schoolyears.stud_id = $stud_id AND ay_id = '$_SESSION[AC]' AND sem_id = '$_SESSION[S]'");
...
} elseif ($row['year_id'] == 3) {
    $pdf->SetFillColor(255,192,203);
} elseif ($row['year_id'] == 4) {
    $pdf->SetFillColor(0,191,255);
}
$pdf ->Cell(30,5,$row['year_abv'] .' - '. $row['course_abv'],1,0,'C',1);
$pdf->SetFont('Arial','B','10');
$pdf ->Cell(15,5,$row['status'],1,1,'C');
$pdf->SetFont('Arial','','10');
$pdf->SetXY(0,44);
$pdf ->Cell(40 ,5,'',0,0);
