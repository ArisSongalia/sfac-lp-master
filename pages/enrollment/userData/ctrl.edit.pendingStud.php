...
if ($_SESSION['role'] == "Adviser" || $_SESSION['role'] == "Admission") {
    if (!empty($_GET['id'])) {
        ...
        if ($_GET['remark'] == "Pending" || $_GET['remark'] == "Canceled") {
            ...
            $action = "Checked";
            // query
            $query = $db->query("UPDATE tbl_schoolyears SET remark = '$action', updatedat = '$date', updatedby = '$_SESSION[name]' WHERE sy_id = '$id'") or die($db->error);
    ...
    if (!empty($_GET['id'])) {
        // remark condition
        if ($_GET['remark'] == "Checked" || $_GET['remark'] == "Disapproved" || $_GET['remark'] == "Pending") {
            // get id by URL
            $id = $_GET['id'];
            $action = "Approved";
            // query
