                        ...
                        <form method="POST">
                            <label class="mt-3">FAQs</label>
                                <select class="form-control" name="name" id="department">
                                    ...
                                    <?php
                                        $status = mysqli_query($db, "SELECT * FROM tbl_schoolyears WHERE ay_id = '$_SESSION[AC]' AND sem_id = '$_SESSION[S]' AND stud_id = '$_SESSION[user_id]'") or die (mysqli_error($db));
                                        $row = mysqli_fetch_array($status);

                                        if ($row['status'] == 'Old') {
                                    ?>
                                        <option value="enroll_old.pdf">How to Enroll for Old Students</option>
                                        <option value="Student-Handbook-SFAC-2023.pdf">View Student Handbook</option>
                                    <?php
                                        } else {
                                    ?>
