...
if (isset($_POST['submit'])) {
    ...
    if (!empty($studNoResult)) {
        ...
        if ($check == 0) {
            ...
            $inquiryData = mysqli_query($db, "INSERT INTO tbl_students (img, stud_no, course_id, gender_id, lastname, firstname, middlename, address, birthdate, birthplace, age, religion, citizenship, civilstatus, contact, email, username, password, curri, created_at, last_updated, updated_by, info_id) VALUES ('', '$stud_no', '$course', '$gender', '$lastname', '$firstname', '$middlename', '$address', '$birthdate', '$birthplace', '$age', '$religion', '$citizenship', '$civilstatus', '$contact', '$email', '$username', '$hashedPwd','',CURRENT_TIMESTAMP,'','', '$info') ") or die(mysqli_error($db));
