...
{   
    ...
    $this->Cell(0,3,'Admiral Village, Talon, Las ' .$test. ' City',0,0,'C');
    // Line break
    $this->Ln(4);
...
$pdf->Cell(51,5,"Last School Att.",1,0);
...
while ($row= mysqli_fetch_array ($query)) {   
    ...
    $tempFontSize = $fontsize;
    $cellwidth = 45;
    $email = utf8_decode($row['lastschool']);

    while ($pdf->GetStringWidth($email) > $cellwidth){
        $pdf->SetFontSize($tempFontSize -= 0.1);
    }
    $pdf->Cell(45,5,utf8_decode($email),0,0);
    $pdf->SetFont('Arial','',9);
    $pdf->Cell(6,5,'',0,0);
